...
- **The same shortcuts across all three editors** (each node only reacts when *its* panel has focus, so a Director + Mockup + Video Edit graph never fights over keys):
  - **Mockup Editor** — `←` `→` nudge the selected layer 1 px (Shift = 10 px); with keyframe(s) selected, `←` `→` move them in time by 1 frame and `↑` `↓` move their position; `S` keys the selected layer at the playhead (the cut); `R` toggles a render window on the key strip (only `[IN → OUT]` renders); `Del` deletes keyframes/layers, `Esc` clears.
  - **Video Edit** — `←` `→` nudge the playhead 1 frame (Shift = 10 frames) for precision cuts; `S` sets a mask key at the playhead; `R` toggles a render window (shaded on the key strip — only that window is processed, mask/reframe keys stay aligned); `Del` removes the mask key at the playhead; `Esc` clears the painted mask.
  - Both render windows are serialized (`render_in`/`render_out`) and **honored in Python**: the Mockup's output video starts at the IN point, and the Video Edit node crops the source to the window and re-times its keys.
- **`?` shortcuts overlay in every editor** — press `?` (or click the **? Help** button in the toolbar) to open a dark panel listing all of that editor's keybindings; `?` or `Esc` closes it (`Esc` only dismisses help when it's open — it never clears your selection/mask as a side effect).
- **Render-range-aware exporting** — the Director node has dedicated **`render_in` / `render_out`** inputs (`-1` = use the timeline's R-key window, or the full timeline). Set them on the node — or hit `R` in the editor (which syncs the widgets) — and **only that window is rendered**: shots, timed refs and seams are clipped and re-based to the window, so the output video starts exactly at IN and nothing outside it is generated (Sony-Vegas-style "render just this scene"). An inverted window (OUT ≤ IN) is rejected with a warning and falls back to the timeline window.
